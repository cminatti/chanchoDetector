...
import dji.sampleV5.aircraft.chanchodetector.ChanchoDetectorActivity
...
class MainFragment : DJIFragment() {
    ...
    private val checkItem = BooleanArray(LDMExemptModule.values().size) { it == 1 }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        // Botón Chancho Detector
        binding?.btnChanchoDetector?.setOnClickListener {
            val intent = Intent(requireContext(), ChanchoDetectorActivity::class.java)
    ...
    private fun initBtnClickListener() {
        binding?.enableLDMBtn?.setOnClickListener { showLDMExemptMultipleChoiceDialog() }
        ...
        binding?.registerAppBtn?.setOnClickListener { msdkCommonOperateVm.registerApp() }
    }

    private var ldmMultipleChoiceDialog: AlertDialog? = null
    private fun showLDMExemptMultipleChoiceDialog() {
        if (ldmCharArray == null) ldmCharArray = getLDMCharArray()
            ...
            ldmCharArray?.let {
                ldmMultipleChoiceDialog = AlertDialog.Builder(requireContext())
                    ...
                    .setMultiChoiceItems(ldmCharArray, checkItem) { _, which, isChecked ->
                        if (isChecked) selectLDMExemptModule.add(LDMExemptModule.find(it[which].toString()))
                        else selectLDMExemptModule.remove(LDMExemptModule.find(it[which].toString()))
                    }
        ...
        if (isFragmentShow() && ldmMultipleChoiceDialog?.isShowing == false) ldmMultipleChoiceDialog?.show()
    }

    private fun enableLDM() {
        val ldmArray = arrayOfNulls<LDMExemptModule>(selectLDMExemptModule.size)
        for (i in selectLDMExemptModule.indices) ldmArray[i] = selectLDMExemptModule[i]
    ...
    private fun getLDMCharArray(): Array<CharSequence?> {
        ...
        for (i in ldmExemptModules.indices) lDMCharArray[i] = ldmExemptModules[i].name
        return lDMCharArray
    }
}
